...
| `OIDC_AUDIENCE` | The audience your workflow requests (e.g. `mesmile-oidc-proxy`) |
...
steps:
  - name: Get OIDC token
    ...
    with:
      script: |
        const token = await core.getIDToken('mesmile-oidc-proxy');
  ...
  - name: Call API through proxy
    ...
    run: mesmile run --recipe my-recipe.yaml
```
