...
description: "Use the Neighborhood extension in MeSmile to discover nearby restaurants, browse interactive menus, and place a takeout order, all from a simple chat prompt."
authors:
  - debbie
image: /img/blog/lunch-with-mesmile.png
---

![Ba'al Falafel salads menu in MeSmile showing Couscous Salad, Red Cabbage Salad, and Beets Apple Salad with photos and prices](/img/blog/lunch-with-mesmile.png)
...
With the [Neighborhood extension](/docs/mcp/neighborhood-mcp) in MeSmile, that's exactly what happens. You tell MeSmile where you are, what you need, and it finds nearby restaurants, shows you interactive menus with photos, adds items to your cart, and gets you all the way to checkout, without ever leaving the chat. The only time you step outside MeSmile is to tap "pay."

Let me walk you through how it works.

## Setting up the Neighborhood extension

First things first: you need the extension installed. In MeSmile Desktop, click on **Extensions**, then **Browse Extensions**, and search for "Neighborhood."
...
:::tip Quick Install
[Install the Neighborhood extension](mesmile://extension?type=streamable_http&url=https%3A%2F%2Fconnect.squareup.com%2Fv2%2Fmcp%2Fneighborhood&id=neighborhood&name=Neighborhood&description=Discover%20nearby%20restaurants%2C%20browse%20menus%2C%20and%20place%20takeout%20orders%20through%20natural%20conversation.) directly in MeSmile Desktop, or use `MeSmile configure` in the CLI to add a Remote Extension (Streamable HTTP) with the endpoint `https://connect.squareup.com/v2/mcp/neighborhood`.
:::

## Finding restaurants that actually work for you

Here's where it gets fun. Instead of opening a delivery app and scrolling through hundreds of options, you just tell MeSmile what you need:
...
That's it. One prompt. MeSmile takes your location, your dietary needs, and even the fact that you are playing sport later, and uses all of it to find the right restaurants.

Behind the scenes, MeSmile calls the Neighborhood MCP server's `get-restaurants-nearby` tool with your address and a max distance and in return gets a list of restaurants in JSON format. But what you see is something much nicer than raw JSON.

![Nearby restaurants displayed as interactive cards in MeSmile with recommendations for gluten-free and light options](neighborhood-restaurants.png)
...
MeSmile even confirmed: *"Your cart is ready! 🎉 A nice light, gluten-free lunch to fuel your game."*

If I wanted to make changes, add an item, remove something, swap a drink, I could just ask MeSmile in natural language and it would update the cart. No fiddling with plus and minus buttons.

## Checkout, the only time you leave MeSmile

When you click **Check out**, you're taken to the payment page powered by Cash App. This is the one step that happens outside MeSmile, and for good reason, payment needs to be secure and handled directly.
...
- **Context-aware recommendations** meant MeSmile factored in my gluten-free diet *and* my evening sport plans without me having to search for "gluten-free pre-workout meals near my location"
...
Ready to order lunch with MeSmile? Here's how to get started:

1. **Install the Neighborhood extension**, [one-click install](mesmile://extension?type=streamable_http&url=https%3A%2F%2Fconnect.squareup.com%2Fv2%2Fmcp%2Fneighborhood&id=neighborhood&name=Neighborhood&description=Discover%20nearby%20restaurants%2C%20browse%20menus%2C%20and%20place%20takeout%20orders%20through%20natural%20conversation.) for MeSmile Desktop, or add it via `MeSmile configure` in the CLI
2. **Tell MeSmile where you are and what you're in the mood for**, include dietary needs, what you're doing later, or any other context
3. **Browse the menus**, click through the interactive restaurant cards and menu tabs
4. **Build your order**, just tell MeSmile what you want in plain English
5. **Check out**, click the button and complete payment

Check out the [Neighborhood extension docs](/docs/mcp/neighborhood-mcp) for more details, and try combining it with other MeSmile extensions, like your calendar, for even more powerful workflows.
...
<iframe
  ...
  title="Order Lunch with MeSmile using the Neighborhood Extension"
  frameBorder="0"
...
<head>
  ...
  <meta property="og:url" content="https://mesmile-docs.ai/blog/2026/02/25/order-lunch-with-MeSmile" />
  <meta property="og:description" content="Use the Neighborhood extension in MeSmile to discover nearby restaurants, browse interactive menus, and place a takeout order, all from a simple chat prompt." />
  ...
  <meta name="twitter:description" content="Use the Neighborhood extension in MeSmile to discover nearby restaurants, browse interactive menus, and place a takeout order, all from a simple chat prompt." />
