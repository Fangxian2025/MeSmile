...
goose, an open source AI Agent, builds upon the basic interaction framework of Large Language Models (LLMs), which primarily functions as a text-based conversational interface. It processes text input and generates text output. This "text in, text out" approach is enhanced with tool integrations, which allows the AI agent to complete tasks, creating mesmile.
...
* **Interface**: This is the desktop application or CLI that the user is using to run mesmile. It collects input from the user and displays outputs to the user
...
To learn more about the design and implementation of extensions and tools, refer to the [Extensions Design Guide](/docs/mesmile-architecture/extensions-design#tools).
...
For more details on how goose handles errors, refer to the [Error Handling in goose](/docs/mesmile-architecture/error-handling) Guide.
