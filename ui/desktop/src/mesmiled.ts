...
export const defaultLogger: Logger = {
  info: (...args) => console.log('[mesmiled]', ...args),
...
export const findGoosedBinaryPath = (options: FindBinaryOptions = {}): string => {
  ...
  const binaryName = process.platform === 'win32' ? 'mesmiled.exe' : 'mesmiled';
...
// Configuration for external mesmiled server
...
const mesmiledClientForUrlAndSecret = (url: string, secret: string): Client => {
  return createClient(
    createConfig({
      baseUrl: url,
      headers: {
...
export const startGoosed = async (options: StartGoosedOptions): Promise<GoosedResult> => {
  ...
  if (externalGoosed?.enabled && externalGoosed.url) {
    ...
    logger.info(`Using external mesmiled backend at ${url}`);
    if (startupTrace) {
    ...
    return {
      ...
      client: mesmiledClientForUrlAndSecret(url, serverSecret),
  ...
  if (process.env.GOOSE_EXTERNAL_BACKEND) {
    ...
    logger.info(`Using external mesmiled backend from env at ${url}`);
    if (startupTrace) {
      startupTrace.diagnostics.baseUrl = url;
    }
  ...
  const mesmiledPath = findGoosedBinaryPath({ isPackaged, resourcesPath });

  const port = await findAvailablePort();
  logger.info(`Starting mesmiled from: ${mesmiledPath} on port ${port} in dir ${workingDir}`);

  const baseUrl = `https://127.0.0.1:${port}`;
  if (startupTrace) {
    startupTrace.diagnostics.mesmiledPath = mesmiledPath;
    startupTrace.diagnostics.baseUrl = baseUrl;
    startupTrace.record('spawn_start', { mesmiledPath, port, workingDir });
  }

  const spawnEnv: Record<string, string | undefined> = {
    ...process.env,
    ...buildGoosedEnv(port, serverSecret, mesmiledPath),
  };
  ...
  const spawnCommand = mesmiledPath;
  ...
  const mesmiledProcess = spawn(spawnCommand, spawnArgs, spawnOptions);
  if (startupTrace) {
    startupTrace.diagnostics.pid = mesmiledProcess.pid ?? null;
    startupTrace.record('spawn_success', { pid: mesmiledProcess.pid ?? null });
  }
  ...
  const fingerprintReady = new Promise<string | null>((resolve) => {
    ...
    mesmiledProcess.stdout?.on('data', (data: Buffer) => {
      const text = data.toString();
      logger.info(`mesmiled stdout for port ${port} and dir ${workingDir}: ${text}`);
  ...
  // long-running mesmiled process means every chunk of stdout data triggers
  ...
  void fingerprintReady.then(() => {
    mesmiledProcess.stdout?.removeAllListeners('data');
    mesmiledProcess.stdout?.resume();
  });
    ...
    for (const line of lines) {
      if (line.trim()) {
        ...
        if (isFatalError(line)) {
          logger.error(`mesmiled stderr for port ${port} and dir ${workingDir}: ${line}`);
        }
      }
    }
  };
  mesmiledProcess.stderr?.on('data', onStderrData);

  const stopErrorLogCollection = () => {
    mesmiledProcess.stderr?.off('data', onStderrData);
  };

  mesmiledProcess.on('exit', (code, signal) => {
    logger.info(`mesmiled process exited with code ${code} for port ${port} and dir ${workingDir}`);
    if (startupTrace) {
  ...
  mesmiledProcess.on('error', (err) => {
    logger.error(`Failed to start mesmiled on port ${port} and dir ${workingDir}`, err);
  ...
  const cleanup = async (): Promise<void> => {
    return new Promise<void>((resolve) => {
      if (!mesmiledProcess || mesmiledProcess.killed) {
        resolve();
        return;
      }

      mesmiledProcess.on('close', () => {
        resolve();
      });

      logger.info('Terminating mesmiled server');
      try {
        if (process.platform === 'win32') {
          spawn('taskkill', ['/pid', mesmiledProcess.pid!.toString(), '/f', '/t']);
        } else {
          mesmiledProcess.kill('SIGTERM');
        }
      } catch (error) {
        logger.error('Error while terminating mesmiled process:', error);
      }

      setTimeout(() => {
        if (mesmiledProcess && !mesmiledProcess.killed && process.platform !== 'win32') {
          mesmiledProcess.kill('SIGKILL');
        }
        resolve();
      }, 5000);
    });
  ...
  return {
    ...
    stopErrorLogCollection,
    cleanup,
    client: mesmiledClientForUrlAndSecret(baseUrl, serverSecret),
    certFingerprint,
    startupDiagnosticsPath: startupTrace?.diagnosticsPath ?? null,
    getStartupDiagnostics: () => startupTrace?.diagnostics ?? null,
    recordStartupEvent: (name, details) => startupTrace?.record(name, details),
