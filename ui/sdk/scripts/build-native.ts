#!/usr/bin/env node
/**
 * Builds the mesmile binary for target platforms and places them
 * into the corresponding npm package directories under ui/mesmile-binary/.
 *
 * Usage:
...
const NATIVE_DIR = resolve(ROOT, "ui/mesmile-binary");
...
function buildTarget(platform: string): void {
  ...
  const pkgDir = resolve(NATIVE_DIR, `mesmile-binary-${platform}`);
  const binDir = resolve(pkgDir, "bin");

  console.log(`==> Building mesmile for ${platform} (${rustTarget})`);
  ...
  try {
    const featureArgs = platform.startsWith("linux-")
      ...
      : "";
    execSync(
      `cargo build --release --target mesmile --bin mesmile${featureArgs}`,
      {
        cwd: ROOT,
        stdio: "inherit",
      },
...
async function main() {
  ...
  console.log("==> Done. Native packages staged in ui/mesmile-binary/");
}
