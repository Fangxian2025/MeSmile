...
A network-level HTTP proxy for simulating provider errors when testing MeSmile's error handling and retry logic.

## Features

- **Interactive error injection**: Manually trigger different error types via stdin commands
- **Network-level interception**: No changes to MeSmile's Rust code required
...
# 2. In another terminal, configure MeSmile to use the proxy
...
# 3. Run MeSmile normally
MeSmile session start "tell me a joke"
...
Then run MeSmile normally. The proxy will intercept API requests and you can manually trigger errors as needed, while authentication and metadata requests are forwarded to the real provider.
...
To use with MeSmile, set these environment variables:
  export OPENAI_HOST=http://localhost:8888
  export ANTHROPIC_HOST=http://localhost:8888
  ...
============================================================
...
2. Configure MeSmile to use the proxy (set environment variables)
3. Run MeSmile in another terminal
4. Use interactive commands to trigger different error types
5. Observe how MeSmile handles each error type
