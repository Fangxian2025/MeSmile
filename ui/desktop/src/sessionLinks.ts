 ...
 * @param url The deep link URL (mesmile://sessions/:shareToken)
...
export async function openSharedSessionFromDeepLink(
  ...
  try {
    if (!url.startsWith('mesmile://sessions/')) {
      throw new Error('Invalid URL: URL must use the mesmile://sessions/ scheme');
    }

    // Extract the share token from the URL
    const shareToken: string = url.replace('mesmile://sessions/', '');
    ...
    setView('sharedSession', {
      sessionDetails: null,
      error: errMsg,
      shareToken: url.replace('mesmile://sessions/', ''),
      baseUrl,
    });
