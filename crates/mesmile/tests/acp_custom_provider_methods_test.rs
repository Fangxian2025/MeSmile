...
use mesmile::config::declarative_providers::load_provider;
use mesmile::config::paths::Paths;
use mesmile::config::{Config, ConfigError, DeclarativeProviderConfig};
...
fn acp_catalog_and_custom_provider_methods_use_core_provider_store() {
    ...
    run_test(async move {
        ...
        for valid_id in ["custom_openai", "openai-compat", "a1"] {
            assert!(
                mesmile::config::declarative_providers::validate_provider_id(valid_id).is_ok(),
