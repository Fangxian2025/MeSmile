...
`.goosehints` is a text file used to provide additional context about your project and improve the communication with mesmile. The use of `.goosehints` ensures that goose understands your requirements better and can execute tasks more effectively.
...
- **Global hints file** - These hints will apply to all your sessions with goose, regardless of directory. Global hints are stored in `~/.config/mesmile/.goosehints`.
...
<Tabs groupId="interface">
    ...
    #### Global hints file
    1. Create a `.goosehints` file in `~/.config/mesmile`
    ...
    - **Global hints file** - Create a `.goosehints` file in `~/.config/mesmile`.
...
Here's how it works:
1. goose looks for each configured filename in both global (`~/.config/mesmile/`) and local project locations
