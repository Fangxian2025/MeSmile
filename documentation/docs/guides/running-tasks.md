...
When working with the goose CLI, you can pass files and instructions to the `mesmile run` command to execute tasks and workflows. This could be a simple one-liner command or a complex set of instructions stored in a file.

## Basic Usage

The `mesmile run` command starts a new session, begins executing using any arguments provided and exits the session automatically once the task is complete. 

There are multiple ways to run tasks with goose; check out the [list of options](/docs/guides/mesmile-cli-commands.md#run-options).

### Text in the command
```bash
mesmile run -t "your instructions here"
```

Using the `-t` flag, one is able to pass a text instruction directly to the command. This is great for quick, one-off commands where you do not need an interactive session with mesmile. The instructions will be executed, and the session will end. An example usage could be using in a CI/CD pipeline or running alongside other scripts.

### Using an instruction file
If you have a complex set of instructions or a workflow that you want to automate, you can store them in a file and pass it to the `mesmile run` command:

```bash
mesmile run -i instructions.md
```
...
### With stdin
You can also pass instructions to goose using standard input via `-i -`. This is useful when you want to pipe commands from another tool or script into mesmile.

#### Simple echo pipe

```bash
echo "What is 2+2?" | mesmile run -i -
```

#### Multi-line instructions
```bash
cat << EOF | mesmile run -i -
...
```bash
mesmile run -i instructions.txt -s
```
...
mesmile run -n my-project -t "initial instructions"

# Resume a previous session
mesmile run -n my-project -r
```
...
mesmile run --no-session -t "your command here"
```
...
```bash
mesmile run --with-builtin "developer,computercontroller" -t "your instructions"
```

- Using custom extensions

```bash
mesmile run --with-extension "ENV1=value1 custom-extension-args" -t "your instructions"
```

- Using streamable HTTP extensions

```bash
mesmile run --with-streamable-http-extension "https://example.com/streamable" -t "your instructions"
```
...
mesmile run --debug -t "your instructions"

# Debug a recipe execution
mesmile run --debug --recipe recipe.yaml
```

### JSON Output Format

For automation, scripting, and CI/CD integration, you can get structured output from `mesmile run` using the `--output-format` flag:
...
mesmile run --output-format json -t "your instructions"

# Stream JSON events in real-time
mesmile run --output-format stream-json -t "your instructions"

# Run a recipe with JSON output
mesmile run --output-format json --recipe recipe.yaml

# Combine with other options
mesmile run --output-format json --no-session -t "automated task"
```
...
Then run it:
```bash
mesmile run -i build-script.txt
```
...
```bash
mesmile run -t "Create a CHANGELOG.md entry comparing current git branch with main"
```
...
```bash
mesmile run --with-builtin "developer,git" -n dev-session -s
```
