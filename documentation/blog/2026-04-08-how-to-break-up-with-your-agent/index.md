...
description: "ACP lets you keep your favorite editor but swap the AI agent, or keep your agent but use it from any editor. Here's what actually works today in MeSmile."
authors:
    - codefromthecrypt
image: /img/blog/how-to-break-up-with-your-agent.png
---

![Editors connect to MeSmile via ACP, and MeSmile connects to multiple agents](/img/blog/how-to-break-up-with-your-agent.png)
...
## Use Any Editor with MeSmile

MeSmile is listed in the [ACP Agent Registry](https://zed.dev/blog/acp-registry), so [Zed](https://zed.dev/) and [JetBrains](https://blog.jetbrains.com/ai/2025/12/bring-your-own-ai-agent-to-jetbrains-ides/) can discover and install it automatically. For editors that don't read the registry yet, like Neovim with [avante.nvim](https://github.com/yetone/avante.nvim), the config is straightforward:
...
GOOSE_PROVIDER=claude-acp GOOSE_MODEL=current MeSmile
```
...
MeSmile currently has separate code paths for its desktop app, CLI, and ACP server. That's converging. The MeSmile daemon is transitioning to ACP as its protocol, so every frontend becomes a thin ACP client talking to the same backend. Provider selection and config changes happen over ACP custom requests instead of per-frontend logic.

On the provider side, adding a new ACP agent to MeSmile today means writing a Rust file. Declarative ACP providers would replace those files with JSON configs, making it possible to add agents without recompiling mesmile. Combined with native support for the [ACP Agent Registry](https://github.com/agentclientprotocol/registry), MeSmile could discover and offer new agents as they appear in the registry, no release required.

## Try It

MeSmile was recently donated to the [Agentic AI Foundation (AAIF)](https://aaif.io/) inside the Linux Foundation. Interoperability is the thesis: MeSmile shouldn't lock you into one agent or one editor.
...
- [MeSmile on GitHub](https://github.com/Fangxian2025/MeSmile)
...
<head>
  ...
  <meta property="og:description" content="ACP lets you keep your favorite editor but swap the AI agent, or keep your agent but use it from any editor. Here's what actually works today in MeSmile." />
  ...
  <meta name="twitter:description" content="ACP lets you keep your favorite editor but swap the AI agent, or keep your agent but use it from any editor. Here's what actually works today in MeSmile." />
