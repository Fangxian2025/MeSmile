...
```bash
mkdir -p ~/.config/mesmile/recipes

curl -sL https://raw.githubusercontent.com/Fangxian2025/MeSmile/main/documentation/src/pages/recipes/data/recipes/ralph-loop.sh -o ~/.config/mesmile/recipes/ralph-loop.sh
curl -sL https://raw.githubusercontent.com/Fangxian2025/MeSmile/main/documentation/src/pages/recipes/data/recipes/ralph-work.yaml -o ~/.config/mesmile/recipes/ralph-work.yaml
curl -sL https://raw.githubusercontent.com/Fangxian2025/MeSmile/main/documentation/src/pages/recipes/data/recipes/ralph-review.yaml -o ~/.config/mesmile/recipes/ralph-review.yaml

chmod +x ~/.config/mesmile/recipes/ralph-loop.sh
```
...
```bash
~/.config/mesmile/recipes/ralph-loop.sh "Create a simple browser using Electron and React"
```
...
```bash
~/.config/mesmile/recipes/ralph-loop.sh ./prd.md
```
:::
...
#   RALPH_RECIPE_DIR      - Recipe directory (default: ~/.config/mesmile/recipes)
#

set -e

INPUT="$1"
RECIPE_DIR="${RALPH_RECIPE_DIR:-$HOME/.config/mesmile/recipes}"
...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    GOOSE_PROVIDER="$WORKER_PROVIDER" GOOSE_MODEL="$WORKER_MODEL" mesmile run --recipe "$RECIPE_DIR/ralph-work.yaml" || {
    ...
    GOOSE_PROVIDER="$REVIEWER_PROVIDER" GOOSE_MODEL="$REVIEWER_MODEL" mesmile run --recipe "$RECIPE_DIR/ralph-review.yaml" || {
