...
- **Global ignore file** - Create a `.gooseignore` file in `~/.config/mesmile`. These restrictions will apply to all your sessions with goose, regardless of directory.
...
1. **Global `.gooseignore`** (applied first)
   - Located at `~/.config/mesmile/.gooseignore`
...
```plaintext
# In ~/.config/mesmile/.gooseignore (global)
