...
REPO="${GOOSE_GITHUB_REPO:-Fangxian2025/MeSmile}"
DEST="$HOME/Downloads"
TMPDIR=$(mktemp -d)
...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/mesmile.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/mesmile.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/mesmile.zip" -d "$TMPDIR/extracted"
