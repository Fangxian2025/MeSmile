...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `mesmile://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('mesmile://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: mesmile://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('mesmile://recipe?config=', '');
