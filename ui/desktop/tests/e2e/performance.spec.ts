...
test.describe('Performance Tests', () => {
  test('measure end-to-end performance for prompt submission', async ({ mesmilePage }) => {
    // Start Playwright tracing to capture all performance data
    await mesmilePage.context().tracing.start({
      screenshots: true,
      snapshots: true,
      sources: true
    });
    ...
    await mesmilePage.evaluate(() => performance.mark('app-ready'));
    ...
    // Mark: Prompt submit
    await mesmilePage.evaluate(() => performance.mark('prompt-submit-start'));
    await chatInput.press('Enter');
    await mesmilePage.evaluate(() => performance.mark('prompt-submitted'));
    ...
    const loadingText = await mesmilePage.locator('[data-testid="loading-indicator"]').textContent();
    if (loadingText?.includes('loading conversation')) {
      await mesmilePage.evaluate(() => performance.mark('loading-conversation-start'));
      console.log('✓ Loading conversation detected');

      // Wait for it to change or disappear
      await mesmilePage.waitForFunction(() => {
      ...
      await mesmilePage.evaluate(() => performance.mark('loading-conversation-end'));
      console.log('✓ Loading conversation complete');
    }

    await mesmilePage.evaluate(() => performance.mark('loading-started'));
    ...
    const checkForFirstToken = async () => {
      while (!firstTokenDetected) {
        try {
          const messageContainers = await mesmilePage.locator('[data-testid="message-container"]').all();
          if (messageContainers.length > 0) {
            const lastMessage = messageContainers[messageContainers.length - 1];
            const content = await lastMessage.textContent();
            if (content && content.trim().length > 0) {
              await mesmilePage.evaluate(() => performance.mark('first-token-received'));
        ...
        await mesmilePage.waitForTimeout(50);
      }
    };
    ...
    await mesmilePage.evaluate(() => performance.mark('response-complete'));
    console.log('✓ Response complete');

    // Create performance measures
    await mesmilePage.evaluate(() => {
    ...
    const metrics = await mesmilePage.evaluate(() => {
    ...
    const response = await mesmilePage.locator('[data-testid="message-container"]').last();
    ...
    await mesmilePage.context().tracing.stop({ path: tracePath });
  ...
  test('measure cold start vs warm cache performance', async ({ mesmilePage }) => {
    await mesmilePage.context().tracing.start({ screenshots: true, snapshots: true });
    ...
    await mesmilePage.evaluate(() => performance.mark('cold-prompt-start'));
    await chatInput.press('Enter');
    ...
    // Second prompt (warm)
    const chatInput2 = await mesmilePage.waitForSelector('[data-testid="chat-input"]');
    await chatInput2.fill('Say goodbye');

    await mesmilePage.evaluate(() => performance.mark('warm-prompt-start'));
  ...
  test('capture full performance profile with navigation timing', async ({ mesmilePage }) => {
    await mesmilePage.context().tracing.start({ screenshots: true, snapshots: true });

    console.log('\n=== Full Performance Profile ===\n');

    // Get navigation timing
    const navigationTiming = await mesmilePage.evaluate(() => {
    ...
    // Wait for app ready
    await mesmilePage.waitForSelector('[data-testid="chat-input"]', { timeout: 30000 });
    await mesmilePage.evaluate(() => performance.mark('app-interactive'));

    // Measure time from navigation to interactive
    const appReadyTime = await mesmilePage.evaluate(() => {
    ...
    await mesmilePage.evaluate(() => performance.mark('user-interaction-start'));
    await chatInput.press('Enter');

    await mesmilePage.waitForSelector('[data-testid="loading-indicator"]', { state: 'visible', timeout: 5000 });
    await mesmilePage.waitForSelector('[data-testid="loading-indicator"]', { state: 'hidden', timeout: 60000 });
    ...
    const interactionTime = await mesmilePage.evaluate(() => {
    ...
    const resourceStats = await mesmilePage.evaluate(() => {
