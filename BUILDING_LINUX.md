...
Output: `out/make/zip/linux/x64/mesmile-linux-x64-{version}.zip`
...
Output: `out/make/deb/x64/mesmile_{version}_amd64.deb`
...
```bash
sudo dpkg -i out/make/deb/x64/mesmile_*.deb
```
...
If you see "Could not find mesmiled binary", ensure you've:
1. Built the Rust backend: `cargo build --release -p mesmile-server`
2. Copied it to the right location: `cp ../../target/release/mesmiled src/bin/`
...
1. **Backend changes**: Rebuild with `cargo build --release -p mesmile-server` and copy the binary
