...
async fn handle_first_time_setup(config: &Config) -> anyhow::Result<()> {
    ...
    println!();
    cliclack::intro(style(" mesmile-configure ").on_cyan().black())?;
...
pub async fn configure_provider_dialog() -> anyhow::Result<bool> {
    ...
    let current_provider: Option<String> = config.get_mesmile_provider().ok();
...

    {
        ...
        if supports_thinking {
            let effort: &str = cliclack::select("Select thinking effort:")
                ...
                .initial_value("off")
                .interact()?;
            config.set_mesmile_thinking_effort(effort)?;
        }
    }
...
pub fn configure_mesmile_mode_dialog() -> anyhow::Result<()> {
    let config = Config::global();
        ...
        Ok(provider) => {
            ...
            match test_result {
                Ok(_) => {
                    ...
                    cliclack::outro("OpenRouter setup complete! You can now use mesmile.")?;
                }
                Err(e) => {
...
fn add_provider() -> anyhow::Result<()> {
    ...
    if !provider_config.models.is_empty() {
        ...
        if let Ok(model) = cliclack::select("Which model should be the default?")
            ...
            .interact()
        {
            config.set_mesmile_provider(&provider_config.name)?;
            config.set_mesmile_model(model)?;
        }
    }
