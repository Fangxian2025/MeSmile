...
impl GooseAcpAgent {
    ...
    pub(super) async fn handle_new_session(
        ...
        let mut builder = self.session_manager.update(&mesmile_session.id);
        let extension_data =
            self.build_enabled_extensions_data(config, &mesmile_session, args.mcp_servers)?;
        builder = builder
        ...
        mesmile_session = self
            .session_manager
            .get_session(&mesmile_session.id, false)
            .await
            .internal_err_ctx("Failed to reload session")?;
        let session_id_str = mesmile_session.id.clone();
        ...
        let (mode_state, model_state, config_options) =
            super::build_session_setup_config(&self.provider_inventory, &mesmile_session).await?;
